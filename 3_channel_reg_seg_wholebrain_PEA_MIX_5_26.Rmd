...
```{r}
wd <- "D:/2019-05-26_PEA_IAMM_MIX_Trap2_cFos/PEA_MIX/PEA_MIX"
setwd(wd)
library(wholebrain)
library(dplyr)
library(naturalsort)
...
Assign bregma coordinates for each image in `r inspected`. SHould be -0.955

```{r}
smp.coord <- c(2.445, 1.42, 0.020, -0.985, -1.555, -3.18)
```
...
You have to specify the indexes for the images you wish to register.
...
```{r}
image <- images[25]
neuron_seg_ch_1 <- segment(input=image, channel=1)
```
...
for(suffix in suffixes){
  seg <- segment(images[suffix], display=FALSE, filter=neuron_seg_ch_1$filter, channel=1)
...
```{r}
image <- images[25]
neuron_seg_ch_2 <- segment(input=image, channel=2)
```

```{r}
for(suffix in suffixes){
  seg <- segment(images[suffix], display=FALSE, filter=neuron_seg_ch_2$filter, channel=2)
  # loads image in output* folders, need to be in working directory
  output <- inspect.registration(get(paste0("regi_", suffix)), seg, soma=TRUE, forward.warps=TRUE, device=FALSE, batch.mode=TRUE)
