...
```{r}
wd <- "D:/2019-05-26_PEA_IAMM_MIX_Trap2_cFos/IAMM_IAMM"
setwd(wd)
library(wholebrain)
library(dplyr)
library(naturalsort)
...
for(registration in file_list){
  load(registration)
  suffix <- as.numeric(as.character(str_match(registration, "image_(.+)_registration")[2]))
...
Channel 1 segmentation, moved files so it wont work? Retain the file structure of Qiang
...
bound_dataset_ch1$animal <- "IAMM_IAMM"
...
bound_dataset_ch2$animal <- "IAMM_IAMM"
...
```{r}
trap2_overlap <- cbind(datasets %>% filter(channel=="trap2"), cum_overlap) %>% filter(cum_overlap==1)
glassbrain(trap2_overlap[grep("Olfactory|olfactory", trap2_overlap$name),], cex=2, high.res=TRUE)
```


```{r}
glassbrain(cbind(datasets %>% filter(channel=="trap2"), cum_overlap) %>% filter(cum_overlap==1), cex=2)
```

```{r}
glassbrain(datasets %>% filter(channel=="cfos"), cex=2)
```

```{r}
schematic.plot(datasets %>% filter(channel=="cfos"),device=FALSE)
```
```{r}
schematic.plot(cbind(datasets %>% filter(channel=="trap2"), cum_overlap) %>% filter(cum_overlap==1),device=FALSE)
```