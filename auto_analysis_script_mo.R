...
wd <- "D:/qiq/2019-01-07_MO_PEA_IAMM_MIX/Flipped/auto_trials/MO"
setwd(wd)
library(wholebrain)
...
inspected <- c(40, 50, 65, 70, 75, 80, 100, 120, 130)
...
smp.coord<-c(2.5, 1.95, 0.75, 0.45, 0.15, -0.25, -1.6, -2.9, -3.9)
...
#datasets <- NULL
#load(file=paste0(wd, '/datasets/', tools::file_path_sans_ext(basename(images[138])), '_accum_dataset.RData'))
...
#datasets_accum$animal <- "MO_auto"
#save(file=paste0(wd, "/datasets/auto_mo_datasets_accum.Robj"), datasets_accum)
write.table(file=paste0(wd, "/datasets/auto_mo_datasets_accum.tsv"), datasets_accum, sep = ",")
